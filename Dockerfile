...
#cd /opt/uwsgi* && \
#make && \
#mv ./uwsgi* /opt/vep/.local/bin/uwsgi*
...
cd /data/project/AStorage/Anfisa-Annotations && pip install -r requirements.txt && cd a_storage/plainrocks && pip3 install Cython && pip3 install . && \
pip3 install -e git+https://github.com/ForomePlatform/forome_misc_tools.git#egg=forome-tools && \
pip3 install pyvcf && pip3 install uwsgi'
