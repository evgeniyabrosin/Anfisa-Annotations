...
public class CompressionOrderValuesTest {
	...
	public void testFromShortAndShort() throws NotSupportCompression {
		...
		byte[] bytes = TypeCompression.ORDER_VALUES.compression.pack(types, expected);

		//Asserts
		for (int index = 0; index < BatchRecord.DEFAULT_SIZE; index++) {
			Object[] values = TypeCompression.ORDER_VALUES.compression.unpackValues(types, bytes, 0, index);
