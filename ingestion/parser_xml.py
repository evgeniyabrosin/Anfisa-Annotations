        ...
        with gzip.open(self.file_name, "rb") as self.file:
            ...
            while True:
                ...
                # Any error while work with block
                    ...
                    self.logger.error('In line %d(set %d)' %
                        (self.line, len(list_of_tuples) + 1))
                    raise
                # Yield batch
...
class XML_File(Parsing_File):
    ...
    def _process_block(self, content):
        ...
        if len(nodes) != 1:
            self.logger.error('%d ReferenceClinVarAssertion elements'
                % len(nodes))
        ...
        for n in nodes:
            for path in self.ref_paths:
                ...
                if type(path) == tuple:
                    ...
                    if len(subn) > 1:
                        self.logger.error(
                            '%d ReferenceClinVarAssertion nodes' % len(subn))
                        raise(BaseException(
                            "Multiple ReferenceClinVarAssertion nodes"))
                    elif len(subn):
                ...
                # For XML content values
                    ...
                    subn = n.xpath(path)
                    if len(subn) > 1:
                        self.logger.error('%d ReferenceClinVarAssertion nodes'
                            % len(subn))
                        raise(BaseException(
                            "Multiple ReferenceClinVarAssertion nodes"))
                    elif len(subn):
        ...
        for n in nodes:
            ...
            for path in self.asr_paths:
                ...
                if type(path) == tuple:
                    subn = n.xpath(path[0])
                    if len(subn) > 1:
                        self.logger.error('%d ClinVarAssertion nodes'
                            % len(subn))
                ...
                # For XML content values
                    ...
                    if len(subn) > 1:
                        self.logger.error('%d ClinVarAssertion nodes'
                            % len(subn))
                        raise(BaseException(
                            "Multiple ClinVarAssertion nodes"))
                    elif len(subn):
    ...
    def get_metadata(self):
        ...
        with gzip.open(self.file_name, "rb") as self.file:
            ...
            root = etree.fromstring(
                bytes(content.getvalue(), encoding='utf-8'), parser)
            # Take referense values
