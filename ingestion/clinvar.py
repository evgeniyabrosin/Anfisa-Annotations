# INSERTING tables ClinVar2Sub_Sig, CV_Submitters, variant_summary, MySQL DATABASE clinvar
# 
import csv
import gzip
...
INSTR_CREATE = """CREATE TABLE IF NOT EXISTS variant_summary( # имя таблицы сделать заглавными буквами?
    AlleleID                 INT(11),
    Type                     TEXT,
    Name                     TEXT,
    GeneID                   INT(11),
...
def ingestCLINVAR(db_host, db_port, user, password, database,
        batch_size, variant_summary_file, XML_FILE):
    ...
    xml = XML_File( file_name = XML_FILE, ref_paths = REFERENCE_PATHS, asr_paths = ASSERTION_PATHS,
    ...
    print(variant_summary_file)
    with gzip.open (variant_summary_file,"rt") as header:
        position = 0
        list_of_records = []
        total, cnt = 0, 0
...
if __name__ == "__main__":
    ingestCLINVAR(
        ...
        database = "clinvar",
        batch_size = 1000,
        variant_summary_file = "/home/trosman/work/clinvar/variant_summary.txt.gz",
        XML_FILE        = "/home/trosman/work/clinvar/ClinVarFullRelease_2020-01.xml.gz")
