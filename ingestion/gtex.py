'''
Creates tables GENE, TISSUE, GENE2TISSUE in db gtex and inserts data into them
...
def ingestGTEX(db_host, db_port, user, password, database,
    ...
    with gzip.open (filename,'rb') as inp:
          gene_records, gen2tis_records,total_gene, total_gen2tis = [],[], 0, 0
          start_time = time.time()
          text_inp = TextIOWrapper(inp, encoding = "utf-8", line_buffering = True)
          line_no = 0
          for line in text_inp:  
             ...
             if line_no > 3:
                ...
                lin = sorted([(-float(fields[i]), i+1) for i in  range(2,len(fields))], key = itemgetter(0))
                if lin[0][0] == 0:
                    continue
                gene_records.append(new_record (fields[0],fields[1], lin[0], lin[1], lin[2]))
                if len(gene_records) >= batch_size:
                    total_gene += execute_insert(conn, INSTR_INSERT_GENE,gene_records) # fill GENE
                    gene_records = []
                ...
                for i in range(len(lin)):
                    if lin[i][0] < 0:
                        gen2tis_records.append([fields[0], lin[i][1], -lin[i][0], lin[i][0]/top])
                    else:
                        break
                if len(gen2tis_records) >= batch_size:
                    total_gen2tis += execute_insert(conn, INSTR_INSERT_GENE2TISSUE, gen2tis_records) 
                    gen2tis_records = []  
                    reportTime("Records_GENE2TISSUE", total_gen2tis, start_time)    
                    
          if len(gene_records) >= 0:
              total_gene += execute_insert(conn, INSTR_INSERT_GENE, gene_records)  
...
if __name__ == '__main__':
    ingestGTEX(
        ...
        database = "gtex",
        batch_size = 10000,
        filename = "/home/trosman/work/GTex/GTEx_Analysis_2017-06-05_v8_RNASeQCv1.1.9_gene_median_tpm.gct.gz")
        # /db/data/GTEx/GTEx_Analysis_2017-06-05_v8_RNASeQCv1.1.9_gene_median_tpm.gct.gz
