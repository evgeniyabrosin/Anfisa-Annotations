...
class SpliceAI_Import(SpliceAI):
    ...
    def ingest(self, vcf):
        ...
        # '/home/trosman/work/whole_genome_filtered_spliceai_scores1000.vcf.gz'
        vcf_reader = pyvcf.Reader(filename = vcf,compressed = True)
            ...
            if len(list_of_values) >= BATCH_SIZE:
                ...
                if cnt >= 10:
                    ...
                    print("Records: {} Time: {}; Rate: {:.2f}".format(
                        total, dt, total / (dt + .0001)))
        if len(list_of_values) > 0:
            total += self.execute_insert(self.insert_sql, list_of_values)
        dt = time.time() - t0
        print("Done: Records {} Time: {}; Rate: {:.2f}".format(
            total, dt, total / (dt + .0001)))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Import VCF files into spliceai database")
    parser.add_argument(
        "--files", "-f", "--input", "-i", required = False, nargs='+',
        help = "Path to files to import",
        default = [
            "/opt/data/gnomad/vcf/exomes/gnomad.exomes.r2.0.2.sites.vcf.bgz"])
    parser.add_argument("--database", default="spliceai",
